...
void ble_ancs_show_attribute(ble_ancs_event_attribute_t *attr)
{
    ...
    if (attr->command_id == 0) {
        list_item.remote_system = BT_NOTIFY_APP_REMOTE_SYSTEM_IOS;
        list_item.notification_type = BT_NOTIFY_APP_LIST_ANCS;
        
      //  list_item.title = {"i","o","s"};
        ...
        for (i = 0; i < attr->attr_num; i++) {
            if (attr->attr_list[i].atrribute_len > 0) {
               ...
               //todo
                ...
                if (attr->attr_list[i].attribute_id == BLE_ANCS_NOTIFICATION_ATTRIBUTE_ID_TITLE) {                           
                    utf8_to_ucs2_string1((uint8_t *)list_item.title, BT_NOTIFY_MAX_PAGE_TITLE_LENGTH * 2, (uint8_t *) attr->attr_list[i].data);
                  //  memcpy(list_item.title, attr->attr_list[i].data, BT_NOTIFY_TITLE_LENTH);

                } else if (attr->attr_list[i].attribute_id == BLE_ANCS_NOTIFICATION_ATTRIBUTE_ID_MESSAGE) {
                //malloc
                   // list_item.data.data = (uint8_t *)pvPortMalloc(attr->attr_list[i].atrribute_len * sizeof(uint8_t));
                   if (attr->attr_list[i].atrribute_len> 1024) {
                       attr->attr_list[i].atrribute_len = 1024;
                   }
                    utf8_to_ucs2_string1((uint8_t *)list_item.data.data, (attr->attr_list[i].atrribute_len + 1) * 2, (uint8_t *) attr->attr_list[i].data);
   
                  //  memcpy(list_item.data.data, attr->attr_list[i].data, attr->attr_list[i].atrribute_len);
                    list_item.data.data_len = attr->attr_list[i].atrribute_len * 2;

                }else if (attr->attr_list[i].attribute_id == BLE_ANCS_NOTIFICATION_ATTRIBUTE_ID_DATE) {
                     memcpy((void *)list_item.data.timestamp, attr->attr_list[i].data, attr->attr_list[i].atrribute_len);
...
void ble_ancs_app_event_callback(ble_ancs_event_t *ancs_evt)
{
    ...
    ble_ancs_message_handler(ancs_evt);
}
