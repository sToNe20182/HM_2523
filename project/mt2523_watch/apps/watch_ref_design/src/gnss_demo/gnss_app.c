...
#define GNSS_FIX_QUALITY_THRESHOLD 30
...
{
    if (call_back) {
        g_disp_callback = call_back;
        GNSSLOGD("callback registered\n");
    } else {
        GNSSLOGD("invalid register\n");
    }
}
...
static void gnss_process_pmtk_response(int8_t*nmea_buf, int32_t buf_len)
{
    ...
    if ( strstr((char*) nmea_buf, "PMTK010") ) {
        if ( strstr((char*) nmea_param[0], "001") ) {
            gnss_app_send_cmd((int8_t*) "PMTK262,3", strlen("PMTK262,3"));
        }
    }
}
...
static void gnss_nmea_data_process(int8_t* nmea_buf, int32_t buf_len) {
    int8_t* nmea_param[60];
    int32_t param_num;
    gnss_location_disp_struct_t loc;
    double lat, lng;
    char command_buf[30];
    static char accuracy_fliter_set;
    static char is_3d_fix;
    static float accuracy;
    if (strstr((char*) nmea_buf, "GGA")) {
        GNSSLOGD("GPGGS parse");
        gnss_get_sentence_param(nmea_buf, buf_len, nmea_param, &param_num);
        GNSSLOGD("string..is fix:%s, lat:%s, long:%s, height:%s\n",
            nmea_param[5], nmea_param[1], nmea_param[3], nmea_param[8]);
        loc.height = atoi((char*) nmea_param[8]);
        lat = atof((char*) nmea_param[1]);
        lng = atof((char*) nmea_param[3]);
        loc.fix_state = atoi((char*) nmea_param[5]);
        GNSSLOGD("num..is fix:%d, lat:%f, long:%f, height:%d\n",
            loc.fix_state, lat, lng, loc.height);
        loc.lat_h = ((int32_t) lat)/100;
        loc.long_h = ((int32_t) lng)/100;
        loc.lat_m = ((int32_t) lat) % 100;
        loc.long_m = ((int32_t) lng) % 100;
        loc.lat_s = (int32_t) ((lat - (int32_t) lat) * 60);
        loc.long_s = (int32_t) ((lng - (int32_t) lng) * 60);
        GNSSLOGD("lat[%d][%d][%d],long[%d][%d][%d]\n",
            loc.lat_h, loc.lat_m, loc.lat_s, loc.long_h, loc.long_m, loc.long_s);
        if (g_disp_callback) {
            GNSSLOGD("call back app\n");
            g_disp_callback(&loc);
        }
    } else if (strstr((char*) nmea_buf, "GSA")) {
        gnss_get_sentence_param(nmea_buf, buf_len, nmea_param, &param_num);
        if (!accuracy_fliter_set && (is_3d_fix = (atoi((char*) nmea_param[1]) == 3))) {
            if (accuracy < GNSS_FIX_QUALITY_THRESHOLD) {
                accuracy_fliter_set = 1;
                sprintf(command_buf, "PMTK328,%d", GNSS_FIX_QUALITY_THRESHOLD);
                gnss_app_send_cmd((int8_t*) command_buf, strlen(command_buf));
            }
        }
    } else if (strstr((char*) nmea_buf, "ACCURACY")) {
        gnss_get_sentence_param(nmea_buf, buf_len, nmea_param, &param_num);
        if (!accuracy_fliter_set && ((accuracy = atof((char*) nmea_param[0])) < GNSS_FIX_QUALITY_THRESHOLD)) {
            if (is_3d_fix == 1) {
                accuracy_fliter_set = 1;
                sprintf(command_buf, "PMTK328,%d", GNSS_FIX_QUALITY_THRESHOLD);
                gnss_app_send_cmd((int8_t*) command_buf, strlen(command_buf));
    ...
    if (strstr((char*) nmea_buf, "GPGGA")) {
        ...
        if (strstr((char*) nmea_buf, "PMTK010,001")) {
            accuracy_fliter_set = 0;
        }
...
static void gnss_task_msg_handler(gnss_message_struct_t *message)
{
    ...
    switch (message->message_id) {
        case GNSS_ENUM_POWER_ON_REQ:
            ...
            if (0) {//(g_gnss_location_context.mode != GNSS_LOCATION_MODE_NONE_PERIODIC) {
