...
SetGPSView::SetGPSView() :
    DemoView<SetGPSPresenter>(),
    onState(CommonService::getGPSStatus()),
    onToggleButtonClicked(this, &SetGPSView::toggleButtonClickedHandler)
{
}
...
void SetGPSView::setupScreen()
{
#ifndef SIMULATOR
    if (onState) {
        LOG_I(tgfx, "Current UI: Settings Set GPS (on)\r\n");
    } else {
        LOG_I(tgfx, "Current UI: Settings Set GPS (off)\r\n");
    }
#endif
    ...
    // Add title
    titleTxt.setTypedText(TypedText(T_GPS_TITLE));
...
void SetGPSView::tearDownScreen()
{
...
void SetGPSView::handleKeyEvent(uint8_t key)
{
...
void SetGPSView::toggleButtonClickedHandler(const AbstractButton &button)
{
    onState = !onState;
    CommonService::setGPSStatus(onState);
    if (onState) {
        buttonTxt.setTypedText(TypedText(T_ON_TEXT));
    } else {
        buttonTxt.setTypedText(TypedText(T_OFF_TEXT));
