...
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t framebuffer[2][400 * 400 * 2];
#ifdef DYNAMIC_BITMAP_LOADER
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t bmpCache[400 * 400 * 2 * 2 + 240 * 1000];
#endif
...
#else
#ifdef DYNAMIC_BITMAP_LOADER
    HAL &hal = touchgfx_generic_init<mt2523HAL>(dma, display, tc, width, height, (uint16_t *)bmpCache, sizeof(bmpCache), 20);
#else
