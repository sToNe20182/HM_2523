...
bool battery_message_allocate(uint32_t *handle)
{
    ...
    LOG_I(bmt_demo, "battery_message_allocate 1  %d  %d \n",battery_context.used_msg_queue_count, APP_TASK_MSG_QUEUE_NUM);
	...
	LOG_I(bmt_demo, "battery_message_allocate 2\n");
    ...
    *handle = msg_queue_num | BATTERY_MSG_SW_MAGIC;
    ...
    if (battery_context.register_callback_flag != true) {
		...
		LOG_I(bmt_demo, "battery_message_allocate3 \r\n");
        if (status != BATTERY_MANAGEMENT_STATUS_OK) {
			LOG_I(bmt_demo, "battery_management_register_callback fail!! status = %d\n\r\n");
