...
void bt_create_task(bool power_on)
{
    bt_mm_init();
    ...
    if (pdPASS != xTaskCreate(bt_task, BT_TASK_NAME, BT_TASK_STACKSIZE/sizeof(StackType_t), NULL, BT_TASK_PRIORITY, NULL)) {
...
bt_bd_addr_t *bt_get_public_addr(void)
{
    return &local_public_addr;
}

bt_bd_addr_t *bt_get_random_addr(void)
{
    uint32_t i;

    for(i = 0; i < 6; i++) {
        * (((uint8_t*)local_random_addr) + i) = (uint8_t)(bt_os_layer_generate_random() & 0xFF);
    }
    * (((uint8_t*)local_random_addr) + 5) |= 0xC0;

    return &local_random_addr;
}
