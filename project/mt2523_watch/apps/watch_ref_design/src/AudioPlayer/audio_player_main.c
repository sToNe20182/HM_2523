        ...
        switch (msg_id) {
            ...
            case AUD_SUSPEND_BY_IND: {
                if (aud_ply->flag & AUDIO_PLAYER_FLAG_START_PLAY) {
                    ...
                    if (aud_ply->a2dp_source_handle != (uint32_t) -1) {
                        bt_source_srv_a2dp_stop(aud_ply->a2dp_source_handle);
                    }
                } else {
...
void audio_player_a2dp_source_callback(bt_source_srv_event_t event_id, void *param)
{
    audio_player_context_t *aud_ply = NULL;
    ...
    switch (event_id) {
        case BT_SOURCE_SRV_EVENT_A2DP_CONNECT: {
            ...
            if (a2dp_connect->result == BT_STATUS_SUCCESS) {
                aud_ply->a2dp_source_handle = a2dp_connect->handle;
                aud_ply->dev = (uint8_t)HAL_AUDIO_DEVICE_NONE;
            }
            break;
        }
        ...
        case BT_SOURCE_SRV_EVENT_A2DP_DISCONNECT: {
            ...
            if (1) {
                aud_ply->a2dp_source_handle = (uint32_t) -1;
                aud_ply->dev = (uint8_t)HAL_AUDIO_DEVICE_HEADSET;
            }
            break;
        }
...
void audio_player_init(void)
{
    ...
    g_aud_cntx.offset = 0;
    g_aud_cntx.a2dp_source_handle = (uint32_t) -1;
    /* create task */
    ...
    bt_source_srv_register_callback(audio_player_a2dp_source_callback);
...
bool audply_has_track_change(void)
{
    audio_player_context_t *aud_ply = NULL;

    aud_ply = audio_player_get_context();

    return aud_ply->track_change;
}


void audply_reset_track_change(void)
{
    audio_player_context_t *aud_ply = NULL;

    aud_ply = audio_player_get_context();

    aud_ply->track_change = false;
}
...
TCHAR *audio_player_get_file_name(void)
{
    ...
    return aud_ply->file_name[aud_ply->file_index];
}
...
int32_t audio_player_set_volume(uint8_t vol)
{
    ...
    LOG_I(AudPly, "[AudPly]set_volume-ami_ret: %d, ret: %d, vol: %d\n",
          ami_ret, ret, aud_ply->vol_lev);
...
int32_t audio_player_mute(void)
{
    ...
    ami_ret = bt_sink_srv_ami_audio_set_mute(aud_ply->aud_id, true, STREAM_OUT);
    aud_ply->vol_lev = (uint8_t) -1;

    LOG_I(AudPly, "[AudPly]set_mute-ami_ret: %d\n", ami_ret);
...
{
    return audply_has_track_change();
}


void audio_player_reset_track_change(void)
{
    audply_reset_track_change();
}
