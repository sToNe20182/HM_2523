...
#define MTKEPO_GPS_SV_NUMBER 32

static int32_t gps_sat_count;

void epo_send_assistance_data_int(int32_t timer_id)
{
    unsigned int *epobuf;
    int8_t strbuf[200];

    if (gps_sat_count < MTKEPO_GPS_SV_NUMBER) {
    } else {
        gnss_stop_timer(timer_id);
        return;
    }
    epobuf = epo_get_single_sv_data(gps_sat_count);
    sprintf((char*) strbuf,
        "PMTK721,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X,%X",
        (unsigned int) gps_sat_count + 1,
...
void gps_timer_epo_send_assistance_data()
{
    gps_sat_count = 0;
    gnss_start_repeat_timer(MTKEPO_ASSIST_EPO_INTERVAL, epo_send_assistance_data_int);
}
