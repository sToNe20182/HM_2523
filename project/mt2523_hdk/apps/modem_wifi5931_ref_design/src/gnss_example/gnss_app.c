...
void gnss_set_aiding_flag(gnss_aiding_flag_enum flag, int32_t is_on)
{
    if (is_on) {
        g_gnss_location_context.aiding_flag |= flag;
    } else {
        g_gnss_location_context.aiding_flag &= (~flag);
    }
}
void gnss_clear_flag_callback()
{
    GNSSLOGD("gnss_clear_flag_callback\n");
    gnss_set_aiding_flag(GNSS_EPO_AIDING_FLAG, 0);
    gnss_set_aiding_flag(GNSS_TIME_AIDING_FLAG, 0);
    gnss_set_aiding_flag(GNSS_LOC_AIDING_FLAG, 0);
    clear_flag_timer_id = -1;
}

int32_t gnss_get_aiding_flag(gnss_aiding_flag_enum flag)
{
    if (g_gnss_location_context.aiding_flag & flag) {
        return 1;
    } else {
        return 0;
    }
...
void gnss_aiding_rtc() {
    hal_rtc_time_t rtc_time = {0};
    char temp_buf[100] = {0};
    int year;
    GNSSLOGD("gnss_aiding_rtc\n");

    if (!gnss_get_aiding_flag(GNSS_TIME_AIDING_FLAG)) {
        GNSSLOGD("rtc aiding flag is not set\n");
        return;
    }
    // note: please set correct UTC time to rtc before here, you can call hal_rtc_set_time() to set rtc.
    hal_rtc_get_time(&rtc_time);
    year = rtc_time.rtc_year + 2000;
    sprintf(temp_buf, 
    ...
    if (year >= 2016) {
        gnss_set_aiding_flag(GNSS_TIME_AIDING_FLAG, 0);
        gnss_app_send_cmd((int8_t*) temp_buf, (int32_t) strlen(temp_buf));
    } else {
        GNSSLOGD("wrong time:%s\n", temp_buf);
    }
}
...
void gnss_process_pmtk_response(int8_t*nmea_buf, int32_t buf_len)
{
    ...
    if ( strstr((char*) nmea_param[0], "PMTK010") ) {
        if ( strstr((char*) nmea_param[1], "002") ) {

            gnss_set_aiding_flag(GNSS_EPO_AIDING_FLAG, 1);
            gnss_set_aiding_flag(GNSS_TIME_AIDING_FLAG, 1);
            gnss_set_aiding_flag(GNSS_LOC_AIDING_FLAG, 1);
            if (clear_flag_timer_id != -1) {
                gnss_stop_timer(clear_flag_timer_id);
            }
            clear_flag_timer_id = gnss_start_timer(120000, (gnss_timer_handle)gnss_clear_flag_callback);
            gnss_aiding_rtc();
            #ifdef GNSS_SUPPORT_EPO
            hal_rtc_time_t rtc_time = {0};
            GNSSLOGD("Aiding EPO\n");
            ...
            #endif
            #endif
            gnss_location_aiding();
        }
    }
}


static void gnss_nmea_sentence_gprmc_process(int8_t* nmea_buf, int32_t buf_len) 
{
    int8_t *nmea_param[40];
    int32_t param_num;
    hal_rtc_time_t rtc_time = {0};
    gnss_get_sentence_param(nmea_buf, buf_len, nmea_param, &param_num);
    if (strstr((char*) nmea_param[2], "A")) {
        rtc_time.rtc_hour = (nmea_param[1][0] - '0') * 10 + (nmea_param[1][1] - '0');
        rtc_time.rtc_min = (nmea_param[1][2] - '0') * 10 + (nmea_param[1][3] - '0');
        rtc_time.rtc_sec = (nmea_param[1][4] - '0') * 10 + (nmea_param[1][5] - '0');
        rtc_time.rtc_day = (nmea_param[9][0] - '0') * 10 + (nmea_param[9][1] - '0');
        rtc_time.rtc_mon = (nmea_param[9][2] - '0') * 10 + (nmea_param[9][3] - '0');
        rtc_time.rtc_year = (nmea_param[9][4] - '0') * 10 + (nmea_param[9][5] - '0');
        hal_rtc_set_time(&rtc_time);
        GNSSLOGD("gprmc, Time:%d-%d-%d %d:%d:%d\n", 
            rtc_time.rtc_year,
            rtc_time.rtc_mon,
            rtc_time.rtc_day,
            rtc_time.rtc_hour,
...
static void gnss_nmea_data_process(int8_t* nmea_buf, int32_t buf_len) {
    if (strstr((char*) nmea_buf, "GPGGA")) {
        ...
        gnss_nmea_sentence_gprmc_process(nmea_buf, buf_len);
...
* @return void
*/
void gnss_app_forward_cmd_helper(int8_t* buf, int32_t buf_len)
{
    int ret_len = 0;
...
* @return void
*/
void gnss_sleep_timer_handle_func(int32_t timer_id)
{
...
void sntp_start_callback(int32_t tid)
{
    GNSSLOGD("sntp_start_callback,tid:%d\n", tid);
    //sntp_setservername(0, "1.hk.pool.ntp.org");
    //sntp_setservername(1, "1.cn.pool.ntp.org");
    sntp_setservername(0, "202.118.1.81");
    sntp_setservername(0, "218.189.210.3");
    sntp_init();
}

void slp_start_callback(int32_t tid)
{
    GNSSLOGD("slp_start_callback,tid:%d\n", tid);
...
void gnss_task_msg_handler(gnss_message_struct_t *message)
{
    ...
    switch (message->message_id) {
        ...
        case GNSS_ENUM_HOST_WAKEUP:
            gnss_wakeup_handle();
            break;
		case GNSS_ENUM_EPO_DOWNLOAD_FINISH:
			epo_download_current_trunk_finish_handle(message->param2);
			break;
        case GNSS_ENUM_SIO_EVENT:
            gnss_sio_event_handler_int((uint32_t)message->param1, message->param2);
            break;
        case GNSS_ENUM_NW_READY:
            gnss_start_timer(2000, sntp_start_callback);
            gnss_start_timer(4000, slp_start_callback);
...
void gnss_demo_app_restart()
{
    gnss_app_forward_cmd((int8_t*) "$PMTK103*30\r\n", (uint32_t) strlen("$PMTK103*30\r\n"));
}
...
{
    gnss_app_forward_cmd(buf, buf_len);
}

void gnss_send_msg(gnss_message_id_enum message_id, int32_t param1, void* param2)
{
	gnss_message_struct_t message;
	message.message_id = message_id;
	message.param1 = param1;
	message.param2 = param2;
	if (gnss_task_cntx.gnss_task_queue && xQueueSend(gnss_task_cntx.gnss_task_queue, &message, 10) == pdPASS) {
	}
}
...
bool is_coclock_enable()
{
    ...
    int *gnss_32k_gpio_setting = (int *) 0xa2020c10;
    int *gnss_clock_frequnce = (int *) 0xa2020e30;
