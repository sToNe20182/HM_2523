...
* @param[in]   bt_noti_data_t: notification data
* @return      void
*/
...
static void epo_recv_data_pack(bt_noti_data_t *noti_data)
{
    ...
    if (g_epo_mem_info.write_ptr >= g_epo_mem_info.start_address
        ...
        if (g_epo_mem_info.write_ptr + noti_data->len <= g_epo_mem_info.end_address) {
            ...
            if (g_epo_mem_info.write_ptr + noti_data->len > ((curr_block + 1) << 12)) {
...
* @return      void
*/
void epo_btnotify_msg_relay(void* data)
{
    ...
    switch (p_data->evt_id) {
        ...
        case BT_NOTIFY_EVENT_DATA: {
            queue_item.data_ptr = (bt_noti_callback_t *)pvPortMalloc(sizeof(bt_noti_callback_t));
            memcpy(queue_item.data_ptr, data, sizeof(bt_noti_callback_t));
            break;
                                   }
        case BT_NOTIFY_EVENT_NOTIFICATION: {
            queue_item.data_ptr = (bt_notify_noti_t *)pvPortMalloc(sizeof(bt_notify_noti_t));
            memcpy(queue_item.data_ptr, data, sizeof(bt_notify_noti_t));
            break;
                                           }
        ...
        case BT_NOTIFY_EVENT_SMS: {
            queue_item.data_ptr = (bt_notify_call_t *)pvPortMalloc(sizeof(bt_notify_call_t));
            memcpy(queue_item.data_ptr, data, sizeof(bt_notify_call_t));
            break;
                                  }
        default: {
            break;
...
* @return      void
*/
void epo_btnotify_msg_hdlr(uint32_t evt_id, void* data)
{
    ...
    switch (evt_id) {
        ...
        case BT_NOTIFY_EVENT_DATA: {
            /*receive data*/
            if (strcmp(p_data->noti_data.sender_id, EPO_EXTCMD_UPDATE_DATA_SENDER) == 0) {
                if (p_data->noti_data.err_code == EPO_FILE_DATA_BEGIN) {
                    GNSSLOGD( "\n[EPO_CB]pack transfer: 0\n");
                    epo_recv_data_start(&(p_data->noti_data));
                    return;
                } else if (p_data->noti_data.err_code == EPO_FILE_DATA_PACK) {
                    GNSSLOGD( "\n[EPO_CB]pack transfer: 1\n");
                    epo_recv_data_pack(&(p_data->noti_data));
                } else if (p_data->noti_data.err_code == EPO_FILE_DATA_END) {
                    ...
                    GNSSLOGD("\n[EPO_CB]error_sender: %s or error_receiver: %s\n", p_data->noti_data.sender_id, p_data->noti_data.receiver_id);
                }
            } else if (strcmp(p_data->noti_data.sender_id, EPO_EXTCMD_UPDATE_MD5_SENDER) == 0) {
                if (p_data->evt_id == BT_NOTIFY_EVENT_DATA) {
                    if (!strcmp(p_data->noti_data.sender_id, EPO_EXTCMD_UPDATE_MD5_SENDER) &&
                        !strcmp(p_data->noti_data.receiver_id, EPO_EXTCMD_UPDATE_MD5_SENDER)) {
                        GNSSLOGD("\n[MD5_CB]md5 data transfer: %s\n", p_data->noti_data.data);
                        is_md5_received = pdTRUE;
                        memcpy(epo_md5, p_data->noti_data.data, p_data->noti_data.len + 1);
