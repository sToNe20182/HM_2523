...
//#define BT_LE_CONNECTION_BUF_SIZE (BT_CONNECTION_MAX* BT_LE_CONNECTION_SIZE)
...
void bt_preread_local_address(bt_bd_addr_t addr)
{
...
void bt_mm_init()
{
    ...
    //bt_memory_init_control_block(BT_MEMORY_CONTROL_BLOCK_L2CAP, l2cap_channel_cb_buf, BT_L2CAP_CHANNEL_BUF_SIZE);
...
bt_bd_addr_ptr_t bt_get_public_addr()
{
    ...
    status = nvdm_read_data_item("BT", "address", buffer, &size);
        if (NVDM_STATUS_OK == status) {
            ...
            for (i = 0; i < 6; ++i) {
                tmp_buf[0] = buffer[2 * i];
                tmp_buf[1] = buffer[2 * i + 1];
                local_public_addr[i] = (uint8_t)strtoul((char *)tmp_buf, NULL, 16);
            }

            LOG_I(common, "[BT]Read address from NVDM [%02X:%02X:%02X:%02X:%02X:%02X]\n", local_public_addr[0],
                  local_public_addr[1], local_public_addr[2], local_public_addr[3], local_public_addr[4], local_public_addr[5]);
            return &local_public_addr;
        } else {
            LOG_I(common, "[BT]Failed to Read from NVDM:%d.\n", status);
            return NULL;
        }
#else
    return (bt_bd_addr_ptr_t)&local_public_addr;
#endif
}
