...
atci_status_t atci_cmd_hdlr_rmt_iperf(atci_parse_cmd_param_t *parse_cmd);
atci_status_t atci_cmd_hdlr_rmt_ping(atci_parse_cmd_param_t *parse_cmd);
#endif
...
#if MTK_WFC_WITH_LWIP_NO_WIFI_ENABLE
    {"AT+RMT_IPERF",     atci_cmd_hdlr_rmt_iperf,         0, 0},
    {"AT+RMT_PING",     atci_cmd_hdlr_rmt_ping,         0, 0},
#endif
};
...
static int scan_event_handler_sample(wifi_event_t event_id, unsigned char *payload, unsigned int len)
{
    ...
    int scan_ap_size_temp = scan_list_temp->list_size;
    ...
    switch (event_id) {
        ...
        case WIFI_EVENT_IOT_SCAN_COMPLETE:
            ...
            if(scan_ap_size_temp > scan_ap_size) {
                return -1;
            }
            for (i = 0; i < scan_ap_size_temp; i++) {
                printf("\nCh\tSSID\t\t\t\t\tBSSID\t\t\tAuth\tCipher\tRSSI\tWPS_EN\tCM\tDPID\tSR\r\n");
                printf("%d\t", scan_ap_list_temp[i].channel);
                printf("%-33s\t", scan_ap_list_temp[i].ssid);
                printf("%02x:%02x:%02x:%02x:%02x:%02x\t",
                       scan_ap_list_temp[i].bssid[0],
                       scan_ap_list_temp[i].bssid[1],
                       scan_ap_list_temp[i].bssid[2],
                       scan_ap_list_temp[i].bssid[3],
                       scan_ap_list_temp[i].bssid[4],
                       scan_ap_list_temp[i].bssid[5]);
                printf("%d\t", scan_ap_list_temp[i].auth_mode);
                printf("%d\t", scan_ap_list_temp[i].encrypt_type);
                printf("%d\t", scan_ap_list_temp[i].rssi);
                printf("%d\t", scan_ap_list_temp[i].is_wps_supported);
                printf("%d\t", scan_ap_list_temp[i].wps_element.configuration_methods);
                printf("%d\t", scan_ap_list_temp[i].wps_element.device_password_id);
                printf("%d\t", scan_ap_list_temp[i].wps_element.selected_registrar);
                printf("\r\n");
                os_memcpy(&scan_ap_list[i], &scan_ap_list_temp[i], sizeof(wifi_scan_list_item_t));
            }

            scan_ap_size = scan_ap_size_temp;
...
uint8_t wifi_set_opmode(uint8_t target_mode)
{
    ...
    wifi_host_config_get_opmode(&origin_op_mode);
    if(target_mode == origin_op_mode) {
        LOGI( "same opmode %d, do nothing", target_mode);
        return 0;
    }
    //lwip_net_stop(origin_op_mode);

    if(wifi_host_config_set_opmode(target_mode) < 0) {
        return 1;
    }
    LOGI("set opmode to [%d]", target_mode);
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+WIFI=xxx,<op>  the handler need to parse the parameters
            ...
            if (param != NULL) {
                if (strstr((char *)parse_cmd->string_ptr, "AT+WIFI=opmode,") != NULL) {
                    ...
                    if (0 == operation) {
                        ret = wifi_host_config_get_opmode(&opmode);
                        if (ret < 0) {
...
#else
        xTaskCreate((TaskFunction_t)iperf_tcp_run_client, IPERF_TASK_NAME, IPERF_TASK_STACKSIZE / sizeof(portSTACK_TYPE), g_iperf_param, IPERF_TASK_PRIO , NULL); //MTK_WFC_WITH_LWIP_NO_WIFI_ENABLE
        //xTaskCreate((TaskFunction_t)iperf_tcp_run_client, IPERF_TASK_NAME, IPERF_TASK_STACKSIZE / sizeof(portSTACK_TYPE), g_iperf_param, 6 , NULL);
#endif
        is_create_task = 1;
    }
...
atci_status_t atci_cmd_hdlr_rmt_ping(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    LOGI("atci_cmd_hdlr_rmt_ping");
    os_memset(&resonse, 0, sizeof(atci_response_t));

    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_TESTING:    // rec: AT+RMT_PING=?
            LOGI("AT TEST OK.\n");
            strcpy((char *)resonse.response_buf, "\r\nOK\r\n");
            resonse.response_len = strlen((char *)resonse.response_buf);
            atci_send_response(&resonse);
