...
static bt_gap_le_local_key_t *bt_app_common_get_local_key(void);
static void bt_app_common_gen_local_key(bt_gap_le_local_key_t *local_key);
//SDP part.
...
static bt_gap_le_local_key_t *bt_app_common_get_local_key(void)
{
    ...
    bt_app_common_gen_local_key(&local_key);
    return &local_key;
}
...
static void bt_app_common_gen_local_key(bt_gap_le_local_key_t *local_key)
{
    ...
    if (NULL != local_key) {
        ...
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_local_key, hal init error!\r\n");
        }
        for (i = 0; i < 30; ++i) {
            ret = hal_trng_get_generated_random_number(&random_seed);
            if (HAL_TRNG_STATUS_OK != ret) {
                LOG_I(BT_DM, "[BT]generate_local_key, generate random number error!\r\n");
            }
        }
        /* LTK generate */
        ret = hal_trng_get_generated_random_number(&random_seed);
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_LTK, generate random number error!\r\n");
        }
        local_key->encryption_info.ltk[0] = random_seed & 0xFF;
        local_key->encryption_info.ltk[1] = (random_seed >> 8) & 0xFF;
        local_key->encryption_info.ltk[2] = (random_seed >> 16) & 0xFF;
        local_key->encryption_info.ltk[3] = (random_seed >> 24) & 0xFF;
        ret = hal_trng_get_generated_random_number(&random_seed);
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_LTK, generate random number error!\r\n");
        }
        local_key->encryption_info.ltk[4] = random_seed & 0xFF;
        local_key->encryption_info.ltk[5] = (random_seed >> 8) & 0xFF;
        local_key->encryption_info.ltk[6] = (random_seed >> 16) & 0xFF;
        local_key->encryption_info.ltk[7] = (random_seed >> 24) & 0xFF;
        ret = hal_trng_get_generated_random_number(&random_seed);
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_LTK, generate random number error!\r\n");
        }
        local_key->encryption_info.ltk[8] = random_seed & 0xFF;
        local_key->encryption_info.ltk[9] = (random_seed >> 8) & 0xFF;
        local_key->encryption_info.ltk[10] = (random_seed >> 16) & 0xFF;
        local_key->encryption_info.ltk[11] = (random_seed >> 24) & 0xFF;
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_LTK, generate random number error!\r\n");
        }
        local_key->encryption_info.ltk[12] = random_seed & 0xFF;
        local_key->encryption_info.ltk[13] = (random_seed >> 8) & 0xFF;
        local_key->encryption_info.ltk[14] = (random_seed >> 16) & 0xFF;
        local_key->encryption_info.ltk[15] = (random_seed >> 24) & 0xFF;
        /* ediv generate */
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_ediv, generate random number error!\r\n");
        }
        local_key->master_id.ediv = random_seed & 0xFFFF;
        /* rand generate */
        ret = hal_trng_get_generated_random_number(&random_seed);
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_rand, generate random number error!\r\n");
        }
        local_key->master_id.rand[0] = random_seed & 0xFF;
        local_key->master_id.rand[1] = (random_seed >> 8) & 0xFF;
        local_key->master_id.rand[2] = (random_seed >> 16) & 0xFF;
        local_key->master_id.rand[3] = (random_seed >> 24) & 0xFF;
        ret = hal_trng_get_generated_random_number(&random_seed);
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_rand, generate random number error!\r\n");
        }
        local_key->master_id.rand[4] = random_seed & 0xFF;
        local_key->master_id.rand[5] = (random_seed >> 8) & 0xFF;
        local_key->master_id.rand[6] = (random_seed >> 16) & 0xFF;
        local_key->master_id.rand[7] = (random_seed >> 24) & 0xFF;

        /* CSRK generate */
        ret = hal_trng_get_generated_random_number(&random_seed);
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_csrk, generate random number error!\r\n");
        }
        local_key->signing_info.csrk[0] = random_seed & 0xFF;
        local_key->signing_info.csrk[1] = (random_seed >> 8) & 0xFF;
        local_key->signing_info.csrk[2] = (random_seed >> 16) & 0xFF;
        local_key->signing_info.csrk[3] = (random_seed >> 24) & 0xFF;
        ret = hal_trng_get_generated_random_number(&random_seed);
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_csrk, generate random number error!\r\n");
        }
        local_key->signing_info.csrk[4] = random_seed & 0xFF;
        local_key->signing_info.csrk[5] = (random_seed >> 8) & 0xFF;
        local_key->signing_info.csrk[6] = (random_seed >> 16) & 0xFF;
        local_key->signing_info.csrk[7] = (random_seed >> 24) & 0xFF;
        ret = hal_trng_get_generated_random_number(&random_seed);
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_csrk, generate random number error!\r\n");
        }
        local_key->signing_info.csrk[8] = random_seed & 0xFF;
        local_key->signing_info.csrk[9] = (random_seed >> 8) & 0xFF;
        local_key->signing_info.csrk[10] = (random_seed >> 16) & 0xFF;
        local_key->signing_info.csrk[11] = (random_seed >> 24) & 0xFF;
        if (HAL_TRNG_STATUS_OK != ret) {
            LOG_I(BT_DM, "[BT]generate_csrk, generate random number error!\r\n");
        }
        local_key->signing_info.csrk[12] = random_seed & 0xFF;
        local_key->signing_info.csrk[13] = (random_seed >> 8) & 0xFF;
        local_key->signing_info.csrk[14] = (random_seed >> 16) & 0xFF;
        local_key->signing_info.csrk[15] = (random_seed >> 24) & 0xFF;
...
static bt_status_t bt_app_common_event_callback_register(void)
{
    ...
    bt_callback_manager_register_callback(bt_callback_type_gap_le_get_local_key, 0, (void *)bt_app_common_get_local_key);
