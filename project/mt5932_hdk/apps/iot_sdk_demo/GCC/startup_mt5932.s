...
/*
 * configure PLL to full speed to because Wi-Fi baseband and RF initialization
 * require correct clock speed.
 */
  ldr r0, =hal_clock_set_pll_dcm_init
  blx r0

/*
 * download Wi-Fi firmware - the parameter should be NULL to avoid invoking
 * the 2nd half of connsys_init because the 2nd half may use varaibles in
 * SYSRAM that are not prepared at this moment.
 */
  mov   r0, #0 
  ldr r1, =connsys_init
  blx r1

/* prepare ROM code memory - in the case of MT5932 non-XIP, this must be done before BSS is initialized */
  ldr r0, =move_iot_rom_data_to_ram
  blx r0
