...
{
	{MODE_PEDO,			MODE_WATCH,		 appWatch_Task},
	{MODE_HR, 			MODE_WATCH,		 appPedo_Task},
	{MODE_BP, 			MODE_WATCH,		 appHR_Task},
	{MODE_HRV, 			MODE_WATCH,		 appBP_Task},
	{MODE_SLEEP, 		MODE_WATCH,		 appHRV_Task},
	{MODE_WATCH,		MODE_WATCH,		 appSLEEP_Task},
};
...
void appKeyProc_Task(void *pParameters) 
{
	...
	for(;;)
	{
		...
		if (pdTRUE == xQueueReceive(queKeypad, &eventGet, portMAX_DELAY))
		{
			...
			{
				case DEVICE_KEY_UP:
					if (eventGet.key_state == HAL_KEYPAD_KEY_RELEASE && (bl_isOn()))
					{
					...
					} else if (eventGet.key_state == HAL_KEYPAD_KEY_LONG_PRESS && (bl_isOn())) {
...
void appMgr_Task(void *pParameters) 
{
	...
	xTaskCreate( appKeyProc_Task,  "appKeyProc_Task", STACK_SIZE_FOR_TASK, NULL, TASK_PRIORITY , NULL);	
	
	modeInUse =  MODE_WATCH;
	xTaskCreate( appModeTable[modeInUse].pAppTask,  "Mode Task", STACK_SIZE_FOR_TASK, NULL, TASK_PRIORITY  , &modeTaskHandle);	
	for(;;)
	{
		if (pdTRUE == xQueueReceive(queAppMgr, &eventGet, portMAX_DELAY))
		{
			...
			{	 
				...
				case EVENT_UPDATE_HR:			// BUTTON 5
					...
					hr_init_val = ((sensor_heart_rate_event_t *)eventGet.userdata)->bpm;
                                        status_init_val = ((sensor_heart_rate_event_t *)eventGet.userdata)->status;
			...
			if (modeChange == 1) 
			{
				if (pdTRUE == xSemaphoreTake(semModeChange, portMAX_DELAY))
				{
						...
						xQueueSend(queAppTask, &outEvent, 0 );
						xTaskCreate( appModeTable[modeInUse].pAppTask,  "Mode Task", STACK_SIZE_FOR_TASK, NULL, TASK_PRIORITY, &modeTaskHandle);	
						//xTaskCreate( appModeTable[modeInUse].pAppTask,  "Mode Task", STACK_SIZE_FOR_TASK, NULL, 3, &modeTaskHandle);	
						
						//xTaskCreate(appAlarm_Task, (const signed char *) "Mode Task", STACK_SIZE_FOR_TASK, NULL, TASK_PRIORITY+1, &modeTaskHandle);	
				} 
			} 
		}
	}
