...
void keypad_powerkey_handler(void);
...
void keypad_task_init(void)
{
    ...
    xTaskCreate( keypad_task_main, "KEYPAD", 2048, NULL, 3, &keypad_task_handle);   
    ...
    if ( keypad_custom_init() == true) {
        ...
        hal_keypad_register_callback((hal_keypad_callback_t)keypad_handler, NULL);
        hal_keypad_enable();
        hal_keypad_powerkey_register_callback((hal_powerkey_callback_t)keypad_powerkey_handler, NULL);
...
void keypad_task_main(void *pvParameters)
{
    ...
    while(1)
    {
        if(xQueueReceive(keypad_queue_handle, &msgs, xTicksToWait)) {
            switch(msgs.msg_id)
            {
