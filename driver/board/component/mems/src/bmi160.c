...
int get_bmi_count_10ms=0;
int get_bmi_count_s=0;

extern unsigned int get_current_time_in_ms(void);

extern RawSensorData sensor_date;

void bmi160_date_get(void)
{
	uint8_t receive[2] = {0};
	uint8_t AccX_L,AccX_H,AccY_L,AccY_H,AccZ_L,AccZ_H;
	uint16_t AccX_M,AccY_M,AccZ_M;
	uint8_t GyrX_L,GyrX_H,GyrY_L,GyrY_H,GyrZ_L,GyrZ_H;
	uint16_t GyrX_M,GyrY_M,GyrZ_M;
	long i;

	sensor_date.sensor_sys_time=get_current_time_in_ms();
	
	bmi160_SPI_read(0x0C,receive);
	GyrX_L = receive[0];receive[0] = 0;
	//LOGI("_______________%d_________\n",GyrX_L);
	//for(i=500000;i;i--);
	bmi160_SPI_read(0x0D,receive);
	GyrX_H = receive[0];receive[0] = 0;
	bmi160_SPI_read(0x0E,receive);
	GyrY_L = receive[0];receive[0] = 0;
	bmi160_SPI_read(0x0F,receive);
	GyrY_H = receive[0];receive[0] = 0;
	bmi160_SPI_read(0x10,receive);
	GyrZ_L = receive[0];receive[0] = 0;
	bmi160_SPI_read(0x11,receive);
	GyrZ_H = receive[0];receive[0] = 0;
	...
	if(get_bmi_count_10ms == 100)
	{
		get_bmi_count_10ms = 0;
		get_bmi_count_s++;
		LOGI("!!!!!!!!!!!!!!!BMI_GET: %d times!!!!!!!!!",get_bmi_count_s);
	}

	if(get_bmi_count_s == 30000)
		get_bmi_count_s = 0;
	*/

	sensor_date.axyz[0] = Convert_Acc_Data(AccX_H,AccX_L);
	sensor_date.axyz[1] = -Convert_Acc_Data(AccY_H,AccY_L);
	sensor_date.axyz[2] = -Convert_Acc_Data(AccZ_H,AccZ_L);

	sensor_date.gxyz[0] = (GyrX_H*256+GyrX_L);
	sensor_date.gxyz[1] = -(GyrY_H*256+GyrY_L);
	sensor_date.gxyz[2] = -(GyrZ_H*256+GyrZ_L);

	//LOGI("_________________________read_160______________");
