...
static void
sio_uart_send_timer_timeout_callback(TimerHandle_t tmr)
{
    LOGW("sio_uart_send_timer_timeout_callback");    
    ...
    if (sio_uart_time_handle) {
        LOGW("resend xTimerStart %d \r\n", sio_uart_time_handle);
        xTimerStart(sio_uart_time_handle, 0);
    }
...
sio_uart_ret_t sio_uart_at_response_handler(uint8_t *payload, uint32_t length, uint8_t is_end)
{
    ...
    if (sio_uart_time_handle) {
        xTimerStop(sio_uart_time_handle, 0);
    }
...
uint32_t sio_uart_send_data(int32_t app_id, const uint8_t *data, uint32_t size)
{
    ...
    if (SIO_UART(rtx_data) == NULL) {
        ...
        if (MSM_STATUS_INACTIVE == msm_query_status_from_modem()) {
            if (MSM_RET_OK != msm_trigger_to_modem(MSM_TRIGGER_TYPE_WAKEUP)) {
                LOGW("wakeup modem fall. \r\n");
                sio_uart_mutex_unlock(SIO_UART(mutex));
                return (uint32_t)SIO_UART_RET_ERROR;
            }
            ...
            msm_gpio_clear_wakeup_to_modem();
        }
#endif
        ...
        if (ret_len == size) {
            ...
            if (sio_uart_time_handle == NULL) {
                sio_uart_time_handle = xTimerCreate("sio uart send timer",
                   10 * 1000 / portTICK_PERIOD_MS,
                   pdFALSE,
                   NULL,
                   sio_uart_send_timer_timeout_callback);
            }
            if (sio_uart_time_handle) {
                xTimerStart(sio_uart_time_handle, 0);
            }
...
sio_uart_ret_t sio_uart_timer_sleep_lock_timeout_handle(void)
{
    sio_uart_ret_t ret = SIO_UART_RET_OK;

    sio_uart_mutex_lock(SIO_UART(mutex));

    LOGW("sio_uart_timer_sleep_lock_timeout_handle. \r\n");
    ...
    sio_uart_timer_delete(SIO_UART_TIMER_SLEEP_LOCK);
